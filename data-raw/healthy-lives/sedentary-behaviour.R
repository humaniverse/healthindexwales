...
lives_sedentary_behaviour <- sedentary_behaviour |>
  ...
  select(ltla23_code = ltla21_code, sedentary_behaviour_percentage, year)
...
hl_sedentary_behaviour <- df |>
  filter(
    str_starts(
      Variable_ItemName_ENG,
      "Active less than 30 minutes in previous week"
    ) &
  ...
  ) |> # Filters column to only include local authority codes
  select(
