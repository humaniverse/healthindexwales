...
# ---- Clean data ----
#Filtered dataset includes GCSES literacy point score for each local authority for academic year 2022/2023
#Literacy point score is calculated by taking the average of the scores for all 2022/2023 GCSE students, using their best score out of GCSE English language, GCSE English Literature, GCSE Welsh language or GCSE Welsh Literature 
...
hl_pupil_attainment_literacy <- df |>
  filter(str_starts(Year_Code, "2023") &
           str_starts(Measure_Code, "LitPoints") & #Filters column to only include literacy score
  ...
  select(ltla21_code = Area_AltCode1,
         `Literacy Point Score` = Data,
         Year = Year_ItemName_ENG) |>
  arrange(ltla21_code)

# ---- Save output to data/ folder ----
usethis::use_data(hl_pupil_attainment_literacy, overwrite = TRUE)
