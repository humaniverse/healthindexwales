...
# Wales Latest LA Population Data
wales_population <- population20_ltla21 |>
...
testing_rate <- testing_number |>
  left_join(wales_hb_ltla, by = c("...2" = "lhb22_name")) |>
  left_join(wales_population) |>
  group_by(ltla21_code) |>
  mutate(testing_rate_per_100k = (`2023` / `total_population`) * 100000) |>
  ungroup() |>
  select(
...
diagnosis_chlamydia <- read_excel("data-raw/healthy-lives/raw-data/sexual_health.xlsx", sheet = 2) |>
  slice(9:15) |>
  mutate(`...2` = case_when(
    `...2` == "ABUHB" ~ "Aneurin Bevan University Health Board",
    `...2` == "BCUHB" ~ "Betsi Cadwaladr University Health Board",
...
diagnosis_gonorrhoea <- read_excel("data-raw/healthy-lives/raw-data/sexual_health.xlsx", sheet = 3) |>
  slice(9:15) |>
  mutate(`...2` = case_when(
    `...2` == "ABUHB" ~ "Aneurin Bevan University Health Board",
    `...2` == "BCUHB" ~ "Betsi Cadwaladr University Health Board",
...
diagnosis_syphilis <- read_excel("data-raw/healthy-lives/raw-data/sexual_health.xlsx", sheet = 4) |>
  slice(9:15) |>
  mutate(`...2` = case_when(
    `...2` == "ABUHB" ~ "Aneurin Bevan University Health Board",
    `...2` == "BCUHB" ~ "Betsi Cadwaladr University Health Board",
...
lives_sexual_health <- diagnosis_chlamydia |>
  ...
  left_join(testing_rate, by = c("ltla21_code" = "ltla24_code")) |>
  select(
    ltla24_code = ltla21_code,
    chlamydia_diagnosis_rate_per_100k,
  ...
  mutate(
    sexual_health_testing_diagnosis_rate_per_100k = (chlamydia_diagnosis_rate_per_100k + gonorrhoea_diagnosis_rate_per_100k + syphilis_diagnosis_rate_per_100k + testing_rate_per_100k) / 4,
    year = "2023"
  ) |>
  ungroup() |>
  select(
