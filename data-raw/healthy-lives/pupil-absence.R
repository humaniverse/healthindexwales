...
primary_absenteeism <- primary_absenteeism_raw |>
  filter(Measure == "Percentage of half-day sessions missed for unauthorised absence")
...
secondary_absenteeism <- secondary_absenteeism_raw |>
  filter(Attribute == "Percentage of half-day sessions missed for unauthorised absence")


# Join absenteeism data
pupil_absenteeism <- primary_absenteeism |>
  left_join(secondary_absenteeism, by = c("Local authority" = "Local authority")) |>
  mutate(`Local authority` = str_remove(`Local authority`, "^The\\s+")) |>
  select(`Local authority`,
    primary_absence = `2023/24.x`,
    secondary_absence = `2023/24.y`
  )
...
lives_pupil_absence <- pupil_absenteeism |>
  ...
  mutate(
    total_pupil_absence_percentage = ((primary_absence + secondary_absence) / 2),
    year = "2023/24"
  ) |>
  ungroup() |>
  select(
