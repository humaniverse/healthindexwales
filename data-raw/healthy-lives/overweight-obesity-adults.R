...
# Overweight and Obesity in Adults data
...
obesity_adults_raw <- read_csv(unzipped_files[2])


obesity_adults <- obesity_adults_raw |>
  filter(
    `Year_Code_INT` == "2024",
    `Register_ItemName_ENG_STR` == "Obesity (patients aged 16+)",
    `Measure_ItemName_ENG_STR` == "Prevalence rate (%)"
  )


# Join datasets
lives_obesity_adults <- obesity_adults |>
  left_join(wales_hb_ltla, by = c("Area_ItemName_ENG_STR" = "lhb22_name")) |>
  filter(!is.na(ltla21_code)) |>
  select(
    ltla25_code = ltla21_code,
    obesity_adults_percentage = Data_DEC,
    year = Year_Code_INT
  )
