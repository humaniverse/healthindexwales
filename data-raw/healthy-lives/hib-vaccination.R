...
# ---- Clean data ----
hl_hib_vaccination_coverage <- df |>
  filter(str_starts(Measure_ItemName_ENG, "Hib") &
  ...
  select("ltla21_code" = `Area_AltCode1`,
         "Hib percentage coverage by 2nd birthday" = `Data`,
         "Year" = `Year_ItemName_ENG`) |>
  arrange(ltla21_code)

# ---- Save output to data/ folder ----
usethis::use_data(hl_hib_vaccination_coverage, overwrite = TRUE)