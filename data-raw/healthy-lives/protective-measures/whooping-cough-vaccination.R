...
# ---- Clean data ----
hl_whooping_cough_vaccination_coverage <- df |>
  filter(str_starts(Measure_ItemName_ENG, "Pertussis") &
           str_starts(Year_Code, "18") & # Filter for most recent data
           str_starts(Area_AltCode1, "W0")) |> # Filter for most recent data
  select("ltla21_code" = `Area_AltCode1`,
         "Whooping cough percentage coverage by 2nd birthday" = `Data`,
         "Year" = `Year_ItemName_ENG`) |>
  arrange(ltla21_code)

# ---- Save output to data/ folder ----
usethis::use_data(hl_whooping_cough_vaccination_coverage, overwrite = TRUE)