...
# ---- Clean data ----
hl_meningitisb_vaccination_coverage <- df |>
  filter(str_starts(Measure_ItemName_ENG, "MeningitisB") &
           str_starts(Year_Code, "18") &
           str_starts(Area_AltCode1, "W0")) |>
  select("ltla21_code" = `Area_AltCode1`,
         "Meningitis B percentage coverage by 2nd birthday" = `Data`,
         "Year" = `Year_ItemName_ENG`) |>
  arrange(ltla21_code)

# ---- Save output to data/ folder ----
usethis::use_data(hl_meningitisb_vaccination_coverage, overwrite = TRUE)