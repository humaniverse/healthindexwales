...
# ---- Clean data ----
hl_mmr_vaccination_coverage <- df |>
  filter(str_starts(Measure_ItemName_ENG, "Measles, mumps and ruebella") &
  ...
  select("ltla21_code" = `Area_AltCode1`,
         "MMR percentage coverage by 2nd birthday" = `Data`,
         "Year" = `Year_ItemName_ENG`) |>
  arrange(ltla21_code)

# ---- Save output to data/ folder ----
usethis::use_data(hl_mmr_vaccination_coverage, overwrite = TRUE)